...
def build_card(
    ...
    *,
    ...
    """
    [BUG FIX] 버튼 value 에 원문 대신 channel + original_ts 만 저장.
    Slack action value 2,000자 제한 초과 방지.
    original_ts: 원본 메시지의 ts (이모지 교체용).
    """
    ...
    # ── 재분류 버튼 ──
    # [BUG FIX] value 에 원문 대신 channel + original_ts 만 포함
...
def _handle_reclassify(body, client, action_id):
    """
    재분류 버튼 공통 처리.
    [BUG FIX] 원본 메시지 ts 로 이모지를 교체하도록 수정.
    [BUG FIX] 원문은 conversations.history 로 재조회.
    """
    ...
    original_ts = payload["original_ts"]    # 원본 메시지 ts
    msg_ts      = body["container"]["message_ts"]  # 봇 답장 메시지 ts
    ...
    # [BUG FIX] 원본 메시지(original_ts)의 이모지를 교체
    try:
...
def send_green_digest(target_hour: int = None, target_channel: str = None):
    """
    [BUG FIX] 채널별 digest_hour 를 존중.
    target_hour 가 지정되면 해당 시각에 해당하는 채널만 전송.
    target_channel 이 지정되면 해당 채널만 전송 (/digest/now 용).
    """
    if target_channel:
        ...
        # 설정 없는 채널 중 기본 시각과 같으면 포함
        if target_hour == DEFAULT_DIGEST_HOUR:
            snapshot = db.pop_green_items()  # 전체 꺼냄
            # 다른 시각 채널 데이터는 복원
            restore = {
...
def send_weekly_report():
    """
    주간 리포트.
    [BUG FIX] DB 집계 사용으로 경쟁 조건 제거.
    [BUG FIX] 분류 0건 채널 건너뜀.
    """
...
def _reschedule_digest():
    """채널별 digest_hour 로 cron job 을 재등록."""
...
def _threshold_option(value: float) -> dict:
    """[BUG FIX] 라벨 교정: 값이 높을수록 2차 검증이 많으므로 '높음'."""
    label_map = {
